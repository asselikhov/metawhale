...
**Решение:**
- 🛠 Упрощена логика определения ATH
- 📊 Бот теперь использует собственную базу данных для отслеживания ATH
- 🔄 Проверяет несколько возможных ID токенов
- 🏆 ATH всегда = max(исторический ATH, текущая цена)
- ♾️ Убрано дублирование логики в разных функциях

### Индикаторы нового ATH
Когда цена достигает нового максимума:
- Отображается 🏆 эмодзи рядом с ATH
- Логируется в консоль "🏆 Обнаружен новый ATH!"
- Сохраняется в базу данных

### Тестирование исправлений
Для проверки логики ATH:
```bash
yarn test-format
```

Для исправления существующих некорректных данных в базе:
```bash
node fix-ath.js
```
