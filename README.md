...
Простой и быстрый Telegram бот для отслеживания цены токена CES.
...
2. В Render Dashboard добавьте секретные переменные окружения:
   - `TELEGRAM_BOT_TOKEN`
   - `MONGODB_URI` 
   - `WEBHOOK_URL`
3. Render автоматически использует настройки из `render.yaml`
